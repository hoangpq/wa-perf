...
/* WEBPACK VAR INJECTION */(function(process) {if (!process.env.LOG_LEVEL) {
...
exports.ensure = (condition, errorMsg) => {
  if (condition) {
    ...
    throw new EnsureError(errorMsg);
  }
};
...
const wrap = __webpack_require__(3);
...
const { ensure, unimplemented } = __webpack_require__(0);
const { typeConversions } = __webpack_require__(5);
...
module.exports = function wrap(exports, fnIdent, argTypes = [], returnType = "()") {
  ...
  return (...args) => {
    ensure(args.length === argTypes.length, `${fnIdent} takes ${argTypes.length} ` + `arguments, ${args.length} given`);

    const transformedArgs = argTypes.map((type, index) => typeConversions[type].arg(args[index], exports));
...
const { ensure, unimplemented } = __webpack_require__(0);
const { newSlice, extractSlice, getSliceData, getStr, POINTER_WIDTH } = __webpack_require__(6);
const { TextDecoder, TextEncoder } = __webpack_require__(1);
...
const { info, trace, debug, ensure } = __webpack_require__(0);
const { TextDecoder, TextEncoder } = __webpack_require__(1);
...
(function(global) {
  ...
  if (typeof module !== "undefined" && module.exports &&
    ...
    global["encoding-indexes"] =
      __webpack_require__(8)["encoding-indexes"];
  }
